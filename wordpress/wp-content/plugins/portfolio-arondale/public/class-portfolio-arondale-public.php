 ...
 */
class Portfolio_Arondale_Public {
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/portfolio-arondale-public.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/portfolio-arondale-public.js', array( 'jquery' ), $this->version, false );
	...
	public function portfolio_pro_output_function( $params ) {
		...
		if ( $portfolio_query->have_posts() ) {
			while ( $portfolio_query->have_posts() ) {
				...
				$localization_name    = $metavalues['localization_name'];
				$client_name   		  = $metavalues['client_name'];
				$value_name    		  = $metavalues['value_name'];
				$scope_name    		  = $metavalues['scope_name'];
				$date_name    		  = $metavalues['date_name'];
				...
				$item_info  .= (!empty($project_name) ? '<h4 class="lgx-item-title"><span class="text">'.$project_name.'</span></h4>' : '');
				$item_info  .= (!empty($localization_name) ? '<p class="lgx-item-intro"><span class="text location-text">'.$localization_name.'</span></p>' : '');
				$item_info  .= (!empty($client_name) ? '<p class="lgx-item-intro"><span class="text details-caption">Builder: </span><span class="text details-text">'.$client_name.'</span></p>' : '');
				$item_info  .= (!empty($value_name) ? '<p class="lgx-item-intro"><span class="text details-caption">Value: </span><span class="text details-text">'.$value_name.'</span></p>' : '');
				$item_info  .= (!empty($scope_name) ? '<p class="lgx-item-intro"><span class="text details-caption">Scope: </span><span class="text details-text">'.$scope_name.'</span></p>' : '');
				$item_info  .= (!empty($date_name) ? '<p class="lgx-item-intro"><span class="text details-caption">Date: </span><span class="text details-text">'.$date_name.'</span></p>' : '');
				$item_info  .= '';
				$item_info .= '</div>';
