...
class Portfolio_Arondale_Admin {
	...
	public function add_plugin_admin_menu() {
		...
		$this->plugin_screen_hook_suffix  = add_submenu_page('edit.php?post_type=portfoliopro', __('Settings', $this->plugin_name), __('Projects Settings', $this->plugin_name ), 'manage_options', 'portfolioprosettings', array($this, 'display_plugin_admin_settings'));
	...
	public function portfoliopro_column( $column_name, $post_id ) {
		...
		$localization  = isset( $fieldValues['localization_name'] ) ? $fieldValues['localization_name'] : '';
		$value		   = isset( $fieldValues['value_name'] ) ? $fieldValues['value_name'] : '';
		$scope		   = isset( $fieldValues['scope_name'] ) ? $fieldValues['scope_name'] : '';
		$date		   = isset( $fieldValues['date_name'] ) ? $fieldValues['date_name'] : '';
		...
		if ($column_name == 'localization_name') {
			echo $localization;
		}
		...
		if ($column_name == 'scope_name') {
			echo $scope;
		}

		if ($column_name == 'date_name') {
			echo $date;
		}
	...
	public function portfoliopro_columns($columns ){
		...
		$columns['client_name']   			 = __('Builder Name', $this->plugin_name);
		$localization['localization_name']   = __('Location', $this->plugin_name);
		$value['value_name'] 				 = __('Value', $this->plugin_name);
		$scope['scope_name'] 				 = __('Scope', $this->plugin_name);
		$date['date_name'] 				 = __('Date', $this->plugin_name);
	...
	public function add_meta_boxes_metabox() {
		...
		add_meta_box(
			'metabox_portfoliopro', __( 'Project Fields', $this->plugin_name ), array(
			$this,
	...
	public function custom_post_type() {
		...
		$labels_portfoliopro = array(
			'name'               => _x( 'Projects', 'Post Type General Name', $this->plugin_name ),
		...
		$args_portfoliopro   = array(
			'label'               => __( 'Projects', $this->plugin_name ),
			'description'         => __( 'Projects Display', $this->plugin_name ),
	...
	public function metabox_portfoliopro_display( $post ) {
		...
		$localization_name    = isset( $fieldValues['localization_name'] ) ? $fieldValues['localization_name'] : '';
		$value_name   		  = isset( $fieldValues['value_name'] ) ? $fieldValues['value_name'] : '';
		$scope_name   		  = isset( $fieldValues['scope_name'] ) ? $fieldValues['scope_name'] : '';
		$date_name   		  = isset( $fieldValues['date_name'] ) ? $fieldValues['date_name'] : '';
		...
		<table class="form-table">
			...
			<tr valign="top">
				...
				<td>
					...
					<p class="description"><?php _e( 'Project Name', $this->plugin_name ); ?></p>
				</td>
			</tr>

			<tr valign="top">
				<td><?php _e( 'Location', $this->plugin_name ) ?></td>
				<td>
					<input type="text" name="metaboxportfoliopro[localization_name]" value='<?php echo $localization_name; ?>'/>
					<p class="description"><?php _e( 'Project Location', $this->plugin_name ); ?></p>
				</td>
			</tr>

			<tr valign="top">
				<td><?php _e( 'Builder', $this->plugin_name ) ?></td>
				<td>
					<input type="text" name="metaboxportfoliopro[client_name]" value='<?php echo $client_name; ?>'/>
					<p class="description"><?php _e( 'Builder Name', $this->plugin_name ); ?></p>
				</td>
			</tr>

			<tr valign="top">
				<td><?php _e( 'Value', $this->plugin_name ) ?></td>
				<td>
					<input type="text" name="metaboxportfoliopro[value_name]" value='<?php echo $value_name; ?>'/>
					<p class="description"><?php _e( 'e.g. $ 50m', $this->plugin_name ); ?></p>
				</td>
			</tr>

			<tr valign="top">
				<td><?php _e( 'Scope', $this->plugin_name ) ?></td>
				<td>
					<input type="text" name="metaboxportfoliopro[scope_name]" value='<?php echo $scope_name; ?>'/>
					<p class="description"><?php _e( 'Scope', $this->plugin_name ); ?></p>
				</td>
			</tr>

			<tr valign="top">
				<td><?php _e( 'Date', $this->plugin_name ) ?></td>
				<td>
					<input type="text" name="metaboxportfoliopro[date_name]" value='<?php echo $date_name; ?>'/>
					<p class="description"><?php _e( 'e.g. January 2018', $this->plugin_name ); ?></p>
				</td>
			</tr>
	...
	public function save_post_metabox_portfoliopro( $post_id, $post ) {
		...
		if ( ! empty( $_POST['metaboxportfoliopro'] ) ) {
			...
			if ( $this->user_can_save( $post_id, 'metaboxportfoliopro', $postData['nonce'] ) ) {
				...
				$saveableData['date_name']          = sanitize_text_field( $postData['date_name'] );
				$saveableData['scope_name']         = sanitize_text_field( $postData['scope_name'] );
				$saveableData['value_name']         = sanitize_text_field( $postData['value_name'] );
				$saveableData['client_name']        = sanitize_text_field( $postData['client_name'] );
				$saveableData['localization_name']  = sanitize_text_field( $postData['localization_name'] );
	...
	public function get_settings_sections() {
		...
		$sections = array(
			array(
				...
				'title' => __('Projects Settings', $this->plugin_name),
				'desc' => '<p class="lgx-update"><strong>'. __('This is default global value for Arondale Projects. Also you can be override from shortcode params.', $this->plugin_name) .'<p><strong>'
			)
		);
